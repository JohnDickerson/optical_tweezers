...
namespace tweezers
{
  ...
  {
    bool bi_interp_lookup ( const Array< blitz::TinyVector<float, 2>, 2 >& force_array,
                            double r, double z, TinyVector<float, 2>& interp_res ) {
      ...
      */
      ...
      cerr << "r = "<< r << "\t";
      cerr << "z = "<< z << "\n";
      cerr << "r_low = " << (i0*(rmax-rmin)) - rmin << ", ";
      cerr << "r_hi = " << (i1*(rmax-rmin)) - rmin<< ", ";  
      cerr << "z_low = " << (j0*(zmax-zmin)) - zmin << ", ";
      cerr << "z_hi = " << (j1*(zmax-zmin)) - zmin << "\n";
      */
