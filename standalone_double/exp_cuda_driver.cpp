...
int main(int argc, char** argv)
{
  ...
  */
  ...
  try{
    ...
    moving_laser = ap["moving_laser"].as<bool>();
    ...
    laser_velocity = make_float3(lexical_cast<float>(split_vec[0]),
    ...
    cerr << "# Max time: " << total_time << " seconds" << endl;
    cerr << "# Timestep: " << constants::time_step * 1e6 << " microsec" << endl;
    cerr << "# Iterations: " << total_iterations << " per particle per run" << endl;
	...
	cerr << "# Num particles " << num_particles << endl;
	cerr << "# Y: [" << y_start << ", " << y_end << "]" << endl;
	cerr << "# Z: [" << z_start << ", " << z_end << "]" << endl;
	cerr << "# Test step: " << test_step * 1e6 << " microns." << endl;
    ...
    GPU_init();
    {
      ...
      size_t s = min( static_cast<size_t>(ceil( sqrt(num_particles) )), 
          ...
          for(z_idx=0; z_idx < total_z_idx; z_idx++)
            {
              ...
              evolve_particle(dimBlock, dimGrid, 
                              states, num_particles, 
                              d_particles, d_lasers, total_iterations);
