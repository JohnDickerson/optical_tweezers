...
namespace tweezers
{
  ...
  public:
    ...
    friend _double2 make_double2(double _x, double _y);
  } double2;

 
  inline double2 make_double2 (double _x, double _y)
  { 
	  double2 d;
	  d.x=_x;
	  d.y=_y; 
	...
	namespace constants
	{
		...
		// Natural constants
		const double3S g = {0,0,9.81};      //gravity, m/s^2  gravity is in positive Z direction
		...
		const double3S langevin_F_g = make_double3S((4.0/3.0) * M_PI * sphere_density * pow(sphere_radius, 3.0),
                                                (4.0/3.0) * M_PI * sphere_density * pow(sphere_radius, 3.0),
                                                (4.0/3.0) * M_PI * sphere_density * pow(sphere_radius, 3.0));

		const double3S langevin_F_b = make_double3S((4.0/3.0) * M_PI * fluid_density * pow(sphere_radius, 3.0),
